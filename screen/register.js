    ...
    <View style={[styles.container, themeContainerStyle]}>
      ...
      <View style={{ flex: 1, alignItems: 'center', backgroundColor: 'white', margin: 32 }}>
        ...
        <View style={{ marginTop: 36.2 }}>
          <InputText secure={false} name="person" plch="Email" value={email}
            onChange={text => setEmail(text)}/>
          <InputText style={{ marginTop: 12 }} secure={true} icn="unlock" plch="Password" value={password}
            onChange={text => setPassword(text)}/>
          <InputText style={{ marginTop: 12 }} secure={true} icn="unlock" plch="Confirm Password" value={confpassword}
            onChange={text => setConfpassword(text)}/>
        </View>

        {/* sign in */}
        <ButtonAuth onPress={()=>{register(email, password, confpassword)}} txt="Sign Up" />

        <View style={styles.text}>
          <Text style={{ color: "#755F86", fontWeight: "400"}}> Already have Account? </Text>
